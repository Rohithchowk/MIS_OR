...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, MenuItem, Select, FormControl, InputLabel, Typography, Container, Box } from '@mui/material';
...
const FilterSearchOR = () => {
  ...
  const fetchBlocks = async () => {
    try {
      const response = await fetch('http://localhost:8010/api/block/blocks');
      if (!response.ok) {
  ...
  const fetchBlockData = async (blockName) => {
    try {
      const response = await fetch(`http://localhost:8010/api/block/data/${blockName}`);
      if (!response.ok) {
  ...
  const fetchDepartments = async (blockName) => {
    try {
      const response = await fetch(`http://localhost:8010/api/block/departments/${blockName}`);
      if (!response.ok) {
  ...
  const fetchDepartmentData = async (departmentName) => {
    try {
      const response = await fetch(`http://localhost:8010/api/block/department/datas/${departmentName}`);
      if (!response.ok) {
  ...
  const fetchCategoriesForBlock = async (blockName) => {
    try {
      const response = await fetch(`http://localhost:8010/api/block/blocks/categories/${blockName}`);
      if (!response.ok) {
  ...
  const fetchCategories = async (blockName, departmentName) => {
    if (departmentName) {
      try {
        const response = await fetch(`http://localhost:8010/api/block/fetchcategories/${blockName}/${departmentName}`);
        if (!response.ok) {
  ...
  const fetchCategoryData = async (blockName, departmentName, categoryName) => {
    try {
      const response = await fetch(`http://localhost:8010/api/block/category/data/${blockName}/${departmentName}/${categoryName}`);
      if (!response.ok) {
  ...
  return (
    <Container>
      <Header variant="h5">Select a Block, Department, and Category</Header>
      <DropdownContainer>
      ...
      {selectedBlock && Object.keys(blockData).length > 0 && !selectedDepartment && (
        <div>
          ...
          {(!selectedTable || selectedTable === 'Faculty') && (
            <div>
              <Typography variant="h3">Faculty</Typography>
              <TableContainer component={Paper}>
                <Table stickyHeader>
                  <TableHead>
                    <TableRow>
...
//   return (
//     <div>
//       <h1>Select a Block, Department, and Category</h1>
//       <div>
...
//       <div key={block._id}>
//         {block.Department.map((department) => (
...
//           <pre>{JSON.stringify(departmentData, null, 2)}</pre>
//         </div>
//       )}
//       {selectedCategory && (
//         <div>
